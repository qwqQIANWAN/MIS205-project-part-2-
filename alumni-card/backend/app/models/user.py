...
from sqlalchemy import String, Boolean, DateTime, Integer, Enum as SAEnum, Text, func
...
class User(Base):
    ...
    alumni_info: Mapped["AlumnusInfo | None"] = relationship(back_populates="user", uselist=False)
    appointments: Mapped[list["Appointment"]] = relationship(back_populates="user")
    activity_registrations: Mapped[list["ActivityRegistration"]] = relationship(back_populates="user")
    association_memberships: Mapped[list["AssociationMember"]] = relationship(back_populates="user")
...
class AlumnusInfo(Base):
    ...
    grade: Mapped[str | None] = mapped_column(String(4), nullable=True)  # 入学年份
    graduation_year: Mapped[str | None] = mapped_column(String(4), nullable=True)
    education: Mapped[str | None] = mapped_column(String(32), nullable=True)  # 本科/硕士/博士
    certificate_image: Mapped[str | None] = mapped_column(String(512), nullable=True)  # 证件照片路径
